# Form UI

`form-ui` contains reusable form components and styles for Angular clients. Its source lives under `libs/form-ui/src/lib`.

## Repo Role

- shared form presentation and interaction primitives
- used across multiple app UIs in the monorepo

## Key Surfaces

- reusable form controls and wrappers
- validation-oriented inputs and field presentation components

## Documentation

- generated API reference in `ui-playground` (Storybook): `/api/form-ui/index.html`, linked from **Docs → API Reference**

## Nx Commands

```bash
pnpm exec nx build form-ui
pnpm exec nx test form-ui
```
