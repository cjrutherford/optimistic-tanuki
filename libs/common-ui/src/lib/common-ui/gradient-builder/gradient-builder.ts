...
build(): string {
    ...
    switch (this.type) {
        ...
        case 'repeating-linear':
            return this.repeatingLinearGradient(colors, direction ?? angle ?? '90deg');
        case 'conic':
            return this.conicGradient(colors, angle ?? '0deg', position ?? '50% 50%');
        case 'repeating-conic':
            return this.repeatingConicGradient(colors, angle ?? '0deg', position ?? '50% 50%');
        case 'radial':
            return this.radialGradient(colors, shape ?? 'ellipse', size ?? '', position ?? '50% 50%');
        case 'repeating-radial':
            return this.repeatingRadialGradient(colors, shape ?? 'ellipse', size ?? '', position ?? '50% 50%');
        default:
            return '';
    }
}
...
private conicGradient(colors: string[], angle: string, position: string): string {
    return `conic-gradient(from ${angle} at ${position}, ${colors.join(', ')})`;
}

private radialGradient(colors: string[], shape: string, size: string, position: string): string {
...
private repeatingConicGradient(colors: string[], angle: string, position: string): string {
    return `repeating-conic-gradient(from ${angle} at ${position}, ${colors.join(', ')})`;
}

private repeatingRadialGradient(colors: string[], shape: string, size: string, position: string): string {
