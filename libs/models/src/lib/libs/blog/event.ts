...
import { IsString, IsOptional, IsDate, IsUUID, MaxLength, MinLength } from 'class-validator';
...
export class CreateEventDto {
    ...
    @ApiProperty({ description: 'Event description', example: 'A conference about...' })
    @IsString()
    @MinLength(10)
    @MaxLength(5000)
    description: string;
    ...
    @ApiProperty({ description: 'Event start time', example: '2026-06-01T09:00:00Z' })
    @IsDate()
    @Type(() => Date)
    startTime: Date;

    @ApiProperty({ description: 'Event end time', example: '2026-06-03T17:00:00Z' })
    @IsDate()
    @Type(() => Date)
    endTime: Date;
