...
import { IsString, IsUUID, IsEnum, IsOptional, MaxLength, MinLength } from 'class-validator';
...
export class CreateRiskDto {
    ...
    @ApiProperty({ description: 'Name of the risk', example: 'Budget overrun risk' })
    @IsString()
    @MinLength(3)
    @MaxLength(200)
    name: string;
    ...
    @ApiPropertyOptional({ description: 'Optional field for a risk resolution, this is to include it in the update dto as well.', enum: RiskResolution })
    @IsOptional()
