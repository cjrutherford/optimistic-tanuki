const ServiceTokens = {
  AUTHENTICATION_SERVICE: 'AUTHENTICATION_SERVICE',
  PROFILE_SERVICE: 'PROFILE_SERVICE',
  SOCIAL_SERVICE: 'SOCIAL_SERVICE',
  ASSETS_SERVICE: 'ASSETS_SERVICE',
  PROJECT_PLANNING_SERVICE: 'PROJECT_PLANNING_SERVICE',
  CHAT_COLLECTOR_SERVICE: 'CHAT_COLLECTOR_SERVICE',
  TELOS_DOCS_SERVICE: 'TELOS_DOCS_SERVICE',
  AI_ORCHESTRATION_SERVICE: 'AI_ORCHESTRATION_SERVICE',
  PROMPT_PROXY: 'PROMPT_PROXY',
  BLOG_SERVICE: 'BLOG_SERVICE',
  PERMISSIONS_SERVICE: 'PERMISSIONS_SERVICE',
  STORE_SERVICE: 'STORE_SERVICE',
  APP_CONFIGURATOR_SERVICE: 'APP_CONFIGURATOR_SERVICE',
  FORUM_SERVICE: 'FORUM_SERVICE',
  FINANCE_SERVICE: 'FINANCE_SERVICE',
  VIDEOS_SERVICE: 'VIDEOS_SERVICE',
  VIDEO_TRANSCODER_WORKER: 'VIDEO_TRANSCODER_WORKER',
  WELLNESS_SERVICE: 'WELLNESS_SERVICE',
  NOTIFICATION_SERVICE: 'NOTIFICATION_SERVICE',
  CLASSIFIEDS_SERVICE: 'CLASSIFIEDS_SERVICE',
  LEAD_SERVICE: 'LEAD_SERVICE',
  SYSTEM_CONFIGURATOR_SERVICE: 'SYSTEM_CONFIGURATOR_SERVICE',
  LEARNING_SERVICE: 'LEARNING_SERVICE',
  AUDIO_WORKSTATION_SERVICE: 'AUDIO_WORKSTATION_SERVICE',
};
export { ServiceTokens };
