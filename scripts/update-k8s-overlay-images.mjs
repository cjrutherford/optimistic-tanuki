...
if (!overlayPathArg || !imageTag) {
  console.error(
    'Usage: node scripts/update-k8s-overlay-images.mjs <overlay-kustomization-path> <image-tag>'
  );
  process.exit(1);
}
...
fs.writeFileSync(
  overlayPath,
  yaml.dump(overlay, { lineWidth: 120, noRefs: true })
);
console.log(
  `Updated ${path.relative(repoRoot, overlayPath)} with ${
    overlay.images.length
  } image overrides.`
);
