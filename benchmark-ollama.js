...
const DISCOVERY_RECOMMENDATIONS = [
  {
    name: 'gemma3:4b',
    desc: "Google's latest. 12B fits TIGHTLY in 8GB. Expect minor offloading.",
  },
  {
    name: 'mistral-nemo:12b-instruct-2407-q3_K_M',
    desc: "NVIDIA's 12B. Fits 8GB perfectly. Great at context.",
  },
  {
    name: 'nemotron-mini:4b',
    desc: "NVIDIA's Edge model. High precision, super fast.",
  },
  {
    name: 'qwen2.5:14b-instruct-q3_K_S',
    desc: '14B logic compressed to ~7GB. Better reasoning than any 8B.',
  },
  {
    name: 'phi3:14b-medium-128k-instruct-q2_K',
    desc: "Running at full precision (FP16) because it's so small.",
  },
  {
    name: 'deepseek-r1:8b',
    desc: 'Best-in-class reasoning/math for this size category.',
  },
];
...
const TESTS = {
  conversation: {
    ...
    messages: [
      {
        role: 'user',
        content: 'Describe a futuristic city in one vivid sentence.',
      },
    ],
    stream: false,
  },
  workflow: {
    ...
    messages: [
      {
        role: 'user',
        content: "Return this data as JSON: {name: 'Kai', id: 5}.",
      },
    ],
    format: 'json',
    stream: false,
...
function formatResult(model, task, tps, note) {
  console.log(
    `| ${model.padEnd(20)} | ${task.padEnd(12)} | ${tps.padEnd(6)} | ${note}`
  );
}
...
async function runSuite() {
  ...
  console.log('\n🚀 STARTING BENCHMARK...');
  console.log(
    `| ${'MODEL'.padEnd(20)} | ${'TASK'.padEnd(12)} | ${'T/S'.padEnd(
      6
    )} | RESULT`
  );
  ...
  for (const model of DISCOVERY_RECOMMENDATIONS.map((m) => m.name)) {
    ...
    const chatRes = await apiRequest('/api/chat', {
      ...TESTS.conversation,
      model,
    });
    ...
    if (
      toolRes.message &&
      toolRes.message.tool_calls &&
      toolRes.message.tool_calls.length > 0
    ) {
