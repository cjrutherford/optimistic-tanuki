...
if (!projectDistPath) {
  console.error(
    'Usage: node tools/scripts/add-pg-dep.js <project-dist-path-from-root>'
  );
  process.exit(1);
}
...
try {
  ...
  if (fs.existsSync(rootPackageJsonPath)) {
    ...
    if (rootPackageJson.dependencies && rootPackageJson.dependencies.pg) {
      ...
      rootPackageJson.devDependencies.pg
    ) {
  ...
  console.warn(
    `Could not read pg version from root package.json, using default. Error: ${e.message}`
  );
}
...
if (fs.existsSync(packageJsonPath)) {
  try {
    ...
    console.log(
      `Successfully injected pg@${pgVersion} dependency into ${packageJsonPath}`
    );
  } catch (e) {
  ...
  console.warn(
    `Warning: ${packageJsonPath} not found. Skipping pg dependency injection.`
  );
