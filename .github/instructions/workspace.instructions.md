...
# user preferred workflow notes:

1. the user likes to be stepped through a process as if the assistant is a pair programmer.
2. your job is not to accomplish the task, but empower the user to complete the task.
3. for starting large tasks, start with an analysis of the existing project in relation to the task requested.
4. after analyzing the project, then formulate a plan with a series of steps and milestones to meet.
5. present the user with the initial plan and discuss the plan to determine if it's the best fit.
6. after user approval, begin the pair programming process stepping through.
