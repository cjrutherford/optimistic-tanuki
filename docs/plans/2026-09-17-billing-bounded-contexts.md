...
- [x] O8. C2: implement §11 R1–R5 via group F (O22–O28). Done 2026-09-22: all of O22–O28 ticked (registry + optional marking; coherence pre-existing; audit table + dedup; telos-contracts + both import migrations; HealthCheck matrix + readiness gate + live e2e; README pattern doc). O8 acceptance = this box.
...
- [x] O21. `/api-docs` covers every new/canonical route; generated clients are the only UI callers for migrated domains (grep for hand `/api/<domain>/*` returns only generated code). Done 2026-09-22: export holds 548 paths / 69 tag families / 0 untagged ops incl. all 10 orval families; caller grep returns only generated code + specs + comments + metadata (mutation matrix, interceptors, configs) + e2e infra + Ollama `/api/chat` (different API) + rss/sitemap URL builders + 2 documented SSR exceptions. Five missed callers found and migrated during the audit (forgeofwill messages, client-interface messages, local-hub city, learning-auth, community-ui create-community).
...
- [x] O22. Orchestrator providers built from the `config.yaml` `dependencies:` map via the
      shared composition helpers (R1–R2); `prompt-proxy` marked optional; no new manifest file. Done 2026-09-22: `ORCHESTRATOR_DEPENDENCY_DEFS` registry (configKey → token/serviceId/required) replaces the hardcoded O24b array; `createOrchestratorProviders(composition, map)` warns on unknown keys; `REQUIRED_ORCHESTRATOR_SERVICE_IDS` + `isRequiredOrchestratorDependency` exported for the O27 gate. Verified: ai-orchestrator 409/409 (7 dependency specs), lint clean.
...
- [x] O25. Per-handler path audit of `chat.gateway.ts` + `profile.controller.ts` AI paths:
      mark each downstream call orchestrated vs direct (R3) in a README table; remove duplicate
      calls on orchestrated flows. Progress 2026-09-22 (S-O25a): audit table landed in
      `apps/ai-orchestrator/README.md` (13 chat.gateway sends + 4 profile AI calls classified;
      only 3 orchestrated calls total, rest are data/identity reads by design). Remaining: O25b dedup.
      Done 2026-09-22 (S-O25b): the only true duplicate was the AI-branch final emit loop
      (identical participant set + payload as the tail fan-out) — removed. Verified: gateway
      1240/1240, lint clean.
- [x] O26. Telos/profile/chat payloads moved to L4/L8/telos-contracts + parity spec (R4). Done 2026-09-22 in 3 slices. O26a: new `libs/telos/contracts` (persona/profile/project DTOs + all three command families, parity spec). O26b: 12 orchestrator files off models/constants onto L4/L8/telos-contracts + new `orchestrator-contract-parity.spec.ts` (ProfileCommands.Get, persona FIND, chat POST_MESSAGE). O26c: gateway chat.gateway + persona controller + chat-collector onto contracts libs; legacy `recipientId` → canonical `recipientIds` (both ends accept both); models barrels re-export (back-compat). Fixes en route: `UpdateProfileDto` rewritten explicitly (PartialType metadata invisible → validation saw only `id`); recovered two DTO files from dist sourcemaps after an overwrite; `@ApiProperty({type:Boolean})` for createChatRoom. Verified: ai-orchestrator 412/412, gateway 1240/1240, chat-collector 53/53, all contracts suites + lints green.
- [x] O27. Extended HealthCheck reply with dependency state + gateway readiness gate (R5);
      `gateway-e2e` covers orchestrator route with a downstream disabled (expects clean
      disabled error, not hang — same `RequestTimeoutInterceptor` bound as §1). Done 2026-09-22
      in 2 slices. O27a: `resolveOrchestratorDependencyStates` + `{status, dependencies}`
      HealthCheck reply (healthy/degraded) + matrix spec. O27b: `AiOrchestrationReadinessInterceptor`
      (interceptor, not guard, so the global timeout bounds the probe) on wellness ai/\*,
      project-planning AI routes and `new_persona_chat` (profile-create and message flows
      excluded: conditional/fire-and-forget AI calls that must not gate their routes); live e2e
      restarts the orchestrator with telos-docs-service omitted (compose override, restored
      after) proving 503 + unavailable message. Verified: ai-orchestrator 417/417, gateway
      1243/1243, e2e 2/2, lint clean, stack restored.
- [x] O28. Docs: orchestrator README records the gateway pattern (manifest → composition →
      single path → contracts) so the next downstream addition follows R1–R5. Done 2026-09-22:
      "Adding a downstream" section in `apps/ai-orchestrator/README.md` (6 steps with owning
      files, contracts libs, interceptor rule, verify commands).
