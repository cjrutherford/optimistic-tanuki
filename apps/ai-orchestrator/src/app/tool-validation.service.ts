...
export class ToolValidationService {
  ...
  validateToolCall(
    ...
    try {
      ...
      if (!parameters || typeof parameters !== 'object') {
        ...
        suggestions.push(
          'Provide parameters as an object with key-value pairs'
        );
      ...
      for (const field of requiredFields) {
        if (
          ...
          (typeof parameters[field] === 'string' &&
            (parameters[field] as string).trim() === '')
        ) {
   ...
   */
  private getToolValidator(
    toolName: string
  ): ((params: ToolParameters) => ToolValidationResult) | null {
    const validators: Record<
      string,
      (params: ToolParameters) => ToolValidationResult
    > = {
  ...
  private validateCreateProject(params: any): ToolValidationResult {
    ...
    if (!params.name || typeof params.name !== 'string') {
      ...
      suggestions.push(
        'Provide a descriptive project name (e.g., "Website Redesign")'
      );
    ...
    if (!params.description) {
      ...
      suggestions.push(
        'Provide a brief description of what this project will accomplish'
      );
    ...
    if (
      ...
      suggestions.push(
        'Use one of these priority values: low, medium, high, critical'
      );
  ...
  private validateCreateTask(params: any): ToolValidationResult {
    ...
    if (!params.title || typeof params.title !== 'string') {
      ...
      suggestions.push(
        'Provide a clear task title describing what needs to be done'
      );
    ...
    if (!params.projectId) {
      ...
      suggestions.push(
        'Specify which project this task belongs to. Use searchProjects first if needed.'
      );
    ...
    if (
      ...
      (typeof params.estimatedHours !== 'number' || params.estimatedHours < 0)
    ) {
      errors.push('estimatedHours must be a positive number');
      suggestions.push(
        'Provide estimated hours as a positive number (e.g., 8, 2.5)'
      );
  ...
  private validateUpdateTask(params: any): ToolValidationResult {
    ...
    if (
      params.status &&
      !['todo', 'in_progress', 'completed', 'blocked'].includes(params.status)
    ) {
      errors.push(
        'status must be one of: todo, in_progress, completed, blocked'
      );
      suggestions.push(
        'Use one of these status values: todo, in_progress, completed, blocked'
      );
  ...
  private validateSearchProjects(params: any): ToolValidationResult {
    ...
    if (
      params.status &&
      !['active', 'completed', 'on_hold', 'cancelled'].includes(params.status)
    ) {
      errors.push(
        'status filter must be one of: active, completed, on_hold, cancelled'
      );
      suggestions.push(
        'Use one of these status values for filtering: active, completed, on_hold, cancelled'
      );
    }
  ...
  private validateGetProfile(params: any): ToolValidationResult {
    ...
    if (!params.profileId && !params.userId) {
      ...
      suggestions.push(
        'Provide either a profileId or userId to fetch profile information'
      );
    }
  ...
  analyzeToolCallError(
    ...
    if (!validation.isValid && validation.retryableErrors?.length > 0) {
      ...
      retryable = false;
      suggestedFix =
        'You may not have permission to perform this action. Please check if you have the required permissions.';
    } else if (
      ...
      retryable = true;
      suggestedFix =
        'Network error occurred. This is usually temporary - you can retry the same request.';
  ...
  private generateRetryInstructions(
    toolName: string,
  ...
  generateToolHelpMessage(toolName: string): string {
    ...
    return (
      toolHelp[toolName] ||
      `Please provide the required information for the ${toolName} action.`
    );
  }
}
