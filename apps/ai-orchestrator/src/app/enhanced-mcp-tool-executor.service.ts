...
export class EnhancedMCPToolExecutor {
  ...
  async executeToolWithRetry(
    ...
    if (!validation.isValid) {
      ...
      return {
        ...
        error: errorMessage,
        retryable:
          validation.retryableErrors && validation.retryableErrors.length > 0,
        suggestedFix:
    ...
    while (attempt <= maxRetries) {
      try {
        ...
        // Success!
        this.logger.log(
          `Tool ${toolName} executed successfully on attempt ${attempt}`
        );
        if (conversationId) {
        ...
        this.logger.warn(
          `Tool ${toolName} failed on attempt ${attempt}: ${lastError}`
        );

        // Analyze the error to determine if it's retryable
        const errorAnalysis = this.toolValidation.analyzeToolCallError(
          toolName,
          parameters,
          lastError
        );
  ...
  async executeToolWithGuidance(
    ...
    const toolResult = await this.executeToolWithRetry(toolName, parameters, {
      conversationId,
    });
    ...
    if (toolResult.success) {
      ...
      if (toolResult.retryable) {
        guidance +=
          ' This error might be temporary - consider retrying with the same parameters.';
      } else {
  ...
  private async executeTool(
    ...
    const toolMethod = (this.mcpExecutor as unknown as Record<string, unknown>)[
      toolName
    ];

    if (typeof toolMethod === 'function') {
      return await (
        toolMethod as (...args: unknown[]) => Promise<unknown>
      ).call(this.mcpExecutor, parameters);
    }
    ...
    for (const methodName of commonMethods) {
      const method = (this.mcpExecutor as unknown as Record<string, unknown>)[
        methodName
      ];
    ...
    throw new Error(
      `Unknown tool: ${toolName}. Available methods: ${Object.getOwnPropertyNames(
        Object.getPrototypeOf(this.mcpExecutor)
      )
        .filter(
          (name) =>
            typeof (
              this.mcpExecutor as unknown as unknown as Record<string, unknown>
            )[name] === 'function'
        )
        .join(', ')}`
    );
  ...
  generateSelfCorrectionPrompt(
    ...
    const analysis = this.toolValidation.analyzeToolCallError(
      toolName,
      parameters,
      error
    );
  ...
  private emitToolCallUpdate(
    ...
    this.logger.debug(
      `Tool call update [${conversationId}]: ${toolName} - ${status}`,
      {
        error,
        attempt,
      }
  ...
  validateAndPrepareParameters(
    toolName: string,
    parameters: ToolParameters
  ): { valid: boolean; message?: string; parameters?: ToolParameters } {
    const validation = this.toolValidation.validateToolCall(
      toolName,
      parameters
    );
