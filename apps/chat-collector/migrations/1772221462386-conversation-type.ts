...
export class ConversationType1772221462386 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `CREATE TYPE "public"."conversation_type_enum" AS ENUM('direct', 'community')`
    );
    await queryRunner.query(
      `ALTER TABLE "conversation" ADD "type" "public"."conversation_type_enum" NOT NULL DEFAULT 'direct'`
    );
    await queryRunner.query(
      `ALTER TABLE "conversation" ADD "communityId" character varying`
    );
    await queryRunner.query(
      `ALTER TABLE "conversation" ADD "ownerId" character varying`
    );
    await queryRunner.query(
      `ALTER TABLE "conversation" ADD "isDeleted" boolean NOT NULL DEFAULT false`
    );
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "conversation" DROP COLUMN "isDeleted"`
    );
    await queryRunner.query(`ALTER TABLE "conversation" DROP COLUMN "ownerId"`);
    await queryRunner.query(
      `ALTER TABLE "conversation" DROP COLUMN "communityId"`
    );
