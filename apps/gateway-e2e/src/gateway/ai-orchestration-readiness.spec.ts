 ...
 * local docker access. Not runnable without them: when the compose project
 * has no ai-orchestration service (e.g. the CI e2e stack, whose manifest
 * excludes it), the suite skips instead of failing.
 */
const COMPOSE_PROJECT = process.env.E2E_COMPOSE_PROJECT || 't3code-baf8592e';
...
function orchestratorRestartSupported(): boolean {
  try {
    return compose('ps ai-orchestration --format json').trim().length > 0;
  } catch {
    return false;
  }
}

const restartSupported = orchestratorRestartSupported();

(restartSupported ? describe : describe.skip)(
  'AI orchestration readiness gate (O27b)',
  () => {
