...
export class RolesController {
    ...
    async addPermissionToRole(@Payload() data: { roleId: string, permissionId: string }) {
        return await this.rolesService.addPermissionToRole(data.roleId, data.permissionId);
    }

    @MessagePattern({ cmd: RoleCommands.RemovePermission })
    async removePermissionFromRole(@Payload() data: { roleId: string, permissionId: string }) {
        return await this.rolesService.removePermissionFromRole(data.roleId, data.permissionId);
    }
    ...
    async getUserRoles(@Payload() data: { profileId: string, appScope?: string }) {
