...
export class Initial1750254462767 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `CREATE TYPE "public"."asset_entity_type_enum" AS ENUM('image', 'video', 'audio', 'document')`
    );
    await queryRunner.query(
      `CREATE TYPE "public"."asset_entity_storagestrategy_enum" AS ENUM('local_block_storage', 'remote_block_storage', 'database_storage')`
    );
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `DROP TYPE "public"."asset_entity_storagestrategy_enum"`
    );
