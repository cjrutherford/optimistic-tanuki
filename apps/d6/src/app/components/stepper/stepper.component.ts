    ...
    <div class="stepper-container">
      <div class="stepper-header">
        <ng-container *ngFor="let q of questions; let i = index">
          <span [class.active]="i === currentStep">{{
            q.label || 'Step ' + (i + 1)
          }}</span>
      ...
      <div class="stepper-content" *ngIf="questions.length > 0">
        <div class="question">
          <ng-container *ngIf="questions[currentStep] as question">
            <label [for]="'question-' + currentStep">{{
              question.question
            }}</label>
      ...
      <div class="stepper-actions">
        ...
        <otui-button [variant]="'secondary'" (action)="processStepAction()">
          Analyze
        </otui-button>
        <otui-button [variant]="'primary'" (action)="nextStep()">
...
export class StepperComponent {
  ...
  nextStep(): void {
    if (!this.questions[this.currentStep]?.canContinue) {
      this.questions[this.currentStep].response =
        'There appears to be something wrong with your response.';
      return;
    }
