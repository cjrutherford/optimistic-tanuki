...
export class PostController {
    constructor(@Inject(ServiceTokens.BLOG_SERVICE) private readonly postService: ClientProxy, private readonly l: Logger) {
        this.l.log('PostController initialized');
        console.log('PostController connecting to postService...');
        this.postService.connect().then(() => {
    ...
    async createPost(@Body() createPost: CreateBlogPostDto) {
        try {
            ...
            throw new HttpException('Failed to create post: [' + error.message + ']', 500);
        }
    }
    ...
    async findAllPosts(@Body() query: BlogPostQueryDto) {
        try {
            const posts = await firstValueFrom(this.postService.send({ cmd: BlogPostCommands.FIND_ALL }, query));
            this.l.log('Posts retrieved successfully');
            return posts;
        } catch (error) {
            this.l.error('Error retrieving posts', error);
            throw new HttpException('Failed to retrieve posts: [' + error.message + ']', 500);
        }
    }
    ...
    async getPost(@Param('id') id: string) {
        try {
            const post = await firstValueFrom(this.postService.send({ cmd: BlogPostCommands.FIND }, id));
            if (!post) {
            ...
            throw new HttpException('Failed to retrieve post: [' + error.message + ']', 500);
        }
    }
    ...
    async updatePost(@Param('id') id: string, @Body() updateData: UpdateBlogPostDto) {
        try {
            ...
            throw new HttpException('Failed to update post: [' + error.message + ']', 500);
        }
    }
    ...
    async deletePost(@Param('id') id: string) {
        try {
            ...
            throw new HttpException('Failed to delete post: [' + error.message + ']', 500);
        }
    }
}
