...
export class FollowController {
  ...
  async follow(@User() user: UserDetails, @Body() followDto: UpdateFollowDto) {
    const followingProfile = await firstValueFrom(
    ...
    const result = await firstValueFrom(
      this.socialClient.send({ cmd: FollowCommands.FOLLOW }, followDto)
    );

    // Broadcast follow event via WebSocket
    if (this.socialGateway && result) {
      this.socialGateway.broadcastFollowEvent(
  ...
  async unfollow(
    ...
    @Body() followDto: UpdateFollowDto
  ) {
    const followingProfile = await firstValueFrom(
      this.profileClient.send(
        { cmd: ProfileCommands.Get },
    ...
    const result = await firstValueFrom(
      this.socialClient.send({ cmd: FollowCommands.UNFOLLOW }, followDto)
    );
  ...
  @ApiResponse({
    status: 201,
    description: 'The followers have been successfully retrieved.',
  })
  @Get('/:id')
  ...
  @ApiResponse({
    status: 201,
    description: 'The following have been successfully retrieved.',
  })
  @Get('/following/:id')
  ...
  @ApiResponse({
    status: 201,
    description: 'The mutual follows have been successfully retrieved.',
  })
  @Get('/mutuals/:id')
  ...
  @ApiResponse({
    status: 201,
    description: 'The following count has been successfully retrieved.',
  })
  ...
  @ApiResponse({
    status: 201,
    description: 'The follower count has been successfully retrieved.',
  })
  @Get('/:id/count')
