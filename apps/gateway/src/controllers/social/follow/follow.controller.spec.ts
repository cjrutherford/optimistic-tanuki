...
describe('FollowController', () => {
  ...
  describe('follow', () => {
    it('should follow a user (happy path)', async () => {
      ...
      expect(module.profileClient.send).toHaveBeenCalledWith(
        { cmd: 'Get:Profile' },
        { id: followDto.followerId }
      );
      expect(module.socialClient.send).toHaveBeenCalledWith(
        { cmd: 'FOLLOW' },
        followDto
      );
    ...
    it('should throw if profile not found', async () => {
      ...
      await expect(controller.follow(user, followDto)).rejects.toThrow(
        'Profile not found'
      );
    });
  ...
  describe('unfollow', () => {
    ...
    it('should throw if profile not found', async () => {
      ...
      const module = controller as any;
      module.profileClient = { send: jest.fn(() => of(null)) };
      await expect(controller.unfollow(user, followDto)).rejects.toThrow(
        'Profile not found'
      );
    });
    it('should throw if userId does not match', async () => {
  ...
  describe('getFollowers', () => {
    it('should get followers', async () => {
      const module = controller as any;
      module.socialClient = {
        send: jest.fn(() => of(['follower1', 'follower2'])),
      };
  ...
  describe('getFollowing', () => {
    it('should get following', async () => {
      const module = controller as any;
      module.socialClient = {
        send: jest.fn(() => of(['following1', 'following2'])),
      };
