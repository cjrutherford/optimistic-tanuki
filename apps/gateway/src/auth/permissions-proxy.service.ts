...
export class PermissionsProxyService {
    ...
    constructor(
        @Inject(ServiceTokens.PERMISSIONS_SERVICE) private readonly permissionsClient: ClientProxy,
        private readonly cacheService: PermissionsCacheService,
    ) {}

    async checkPermission(profileId: string, permission: string, appScopeName: string): Promise<boolean> {
        try {
            ...
            this.logger.error(`Error checking permission ${permission} for profile ${profileId}`, error);
            return false;
        }
    }
}
