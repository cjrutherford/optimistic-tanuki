...
import { ContactService, SpamProtectionService } from '../services';
...
export class ContactController {
  ...
  async createContact(
    @Payload() createContactDto: CreateContactDto & { honeypot?: string }
  ): Promise<ContactDto> {
    this.l.log('Starting createContact');
    try {
      // Validate contact data
      const validation =
        this.spamProtectionService.validateContactData(createContactDto);
      if (!validation.isValid) {
        this.l.warn('Contact validation failed', validation.errors);
      ...
      // Check for spam
      const spamCheck =
        this.spamProtectionService.checkForSpam(createContactDto);
      if (spamCheck.isSpam) {
        this.l.warn('Contact submission detected as spam', spamCheck.reason);
        throw new RpcException({
          statusCode: 400,
  ...
  async updateContact(
    @Payload() data: { id: string; updateContactDto: UpdateContactDto }
  ): Promise<ContactDto> {
