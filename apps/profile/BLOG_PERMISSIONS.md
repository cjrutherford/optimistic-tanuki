...
```typescript
// Set blog role
await profileService.send({ cmd: ProfileCommands.SetBlogRole }, { profileId: 'profile-id-here', blogRole: BlogRole.POSTER });

// Get blog role
const role = await profileService.send({ cmd: ProfileCommands.GetBlogRole }, 'user-id-here');
```
