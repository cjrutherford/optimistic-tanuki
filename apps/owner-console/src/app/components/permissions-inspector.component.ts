  ...
  template: `
    <div class="permissions-inspector">
      ...
      <div class="controls">
        <div class="form-group">
          ...
          <select
            ...
            (ngModelChange)="onProfileIdChange($event)"
            class="form-control"
          >
            <option value="">-- Select a user --</option>
            @for (profile of profiles(); track profile.id) {
            <option [value]="profile.id">
              {{ profile.profileName }} ({{ profile.userId }}) -
              {{ profile.appScope }}
            </option>
            }
          </select>
      ...
      <div class="permission-details">
        <h3>
          Permission Details for {{ selectedPermissionInfo()?.profileName }}
        </h3>
        ...
        <div class="details-grid">
          <div class="detail-section">
            ...
            <p>
              ...
              {{ selectedPermissionInfo()?.profileId }}
            </p>
            <p>
              <strong>App Scope:</strong>
              {{ selectedPermissionInfo()?.appScope }}
            </p>
          </div>
          ...
          <div class="detail-section">
            <h4>
              Assigned Roles ({{
                selectedPermissionInfo()?.roles?.length ?? 0
              }})
            </h4>
            <ul>
          ...
          <div class="detail-section">
            <h4>
              Effective Permissions ({{
                selectedPermissionInfo()?.permissions?.length ?? 0
              }})
            </h4>
            <div class="permissions-list">
              @for (permission of selectedPermissionInfo()?.permissions; track
              permission) {
          ...
          <div class="detail-section">
            <h4>
              Allowed Routes ({{
                selectedPermissionInfo()?.allowedRoutes?.length ?? 0
              }})
            </h4>
            <div class="routes-list">
              @for (route of selectedPermissionInfo()?.allowedRoutes; track
              route) {
              <code>{{ route }}</code>
              } @empty {
...
export class PermissionsInspectorComponent extends Themeable implements OnInit {
  ...
  onProfileIdChange(id: string) {
    this.selectedProfileId.set(id);
    this.onProfileChange();
  }

  applyTheme(colors: ThemeColors): void {
    // Optional: manually update grid theme if needed, but [theme] input handles it for quartz
  }
  ...
  async inspectProfile(profileId: string): Promise<void> {
    ...
    try {
      ...
      const roles = await firstValueFrom(
        this.rolesService.getUserRoles(profileId)
      );
      ...
      const allowedRoutes = this.mapPermissionsToRoutes(
  ...
  async inspectAllProfiles(): Promise<void> {
    ...
    try {
      ...
      const profilePromises = profiles.map(async (profile) => {
        try {
          const roles = await firstValueFrom(
            this.rolesService.getUserRoles(profile.id)
          );
      ...
      const successfulResults = allData
        .filter(
          (result): result is PromiseFulfilledResult<UserPermissionInfo> =>
  ...
  private mapPermissionsToRoutes(permissions: string[]): string[] {
    ...
    const permissionRouteMap: { [key: string]: string[] } = {
      ...
      'blog.post.read': [
        'GET /api/blogging/post/:id',
        'GET /api/blogging/post',
      ],
