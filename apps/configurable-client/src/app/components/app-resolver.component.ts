...
@Component({
  ...
  template: `
    ...
    <div class="error-container">
      ...
      <p class="error-hint">
        Please check that the application configuration exists.
      </p>
    </div>
    } @else {
...
export class AppResolverComponent implements OnInit {
  ...
  private loadConfiguration(): void {
    ...
    if (appNameParam) {
      console.log(
        '[AppResolver] Loading configuration by route parameter:',
        appNameParam
      );
    ...
    this.route.queryParams.subscribe((params) => {
      ...
      if (appNameQuery) {
        console.log(
          '[AppResolver] Loading configuration by query parameter:',
          appNameQuery
        );
  ...
  private loadByName(name: string): void {
    this.configService.getConfigurationByName(name).subscribe({
      ...
      error: (err) => {
        ...
        this.error = `Failed to load application configuration for "${name}". ${
          err.status === 404 ? 'Configuration not found.' : 'Server error.'
        }`;
        this.loading = false;
      },
    });
  ...
  private loadByDomain(domain: string): void {
    this.configService.getConfigurationByDomain(domain).subscribe({
      next: (config) => {
        console.log(
          '[AppResolver] Configuration loaded successfully by domain:',
          config
        );
        this.configService.setConfiguration(config);
        void this.tenantTheme.apply(config.theme).finally(() => {
      ...
      error: (err) => {
        console.error(
          '[AppResolver] Failed to load configuration by domain:',
          domain,
          err
        );
        ...
        this.route.queryParams.subscribe((params) => {
          ...
          if (appNameQuery) {
            console.log(
              '[AppResolver] Falling back to query parameter:',
              appNameQuery
            );
