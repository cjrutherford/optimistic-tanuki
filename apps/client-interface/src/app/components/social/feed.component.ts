...
export class FeedComponent implements OnInit, OnDestroy {
  ...
  commented(newComment: CreateCommentDto, postIndex: number) {
    ...
    console.log('🚀 ~ FeedComponent ~ commented ~ assigning profileId:', newComment.profileId);
    ...
    this.commentService.createComment(newComment).subscribe({
      next: (comment) => {
        ...
        if (post) {
          ...
          console.log('Posts array updated immutably. Comments count:', post.comments.length);
        }
      },
      error: (error) => {
