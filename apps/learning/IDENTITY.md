...
**Typography, shape, shadow, and structural tokens.** `--lx-font-heading` and
`--lx-font-mono` are not literals, they are adapters onto the personality:
heading resolves through `var(--font-heading, var(--personality-font-family, inherit))`,
mono through `var(--font-mono, ui-monospace, monospace)`. Under `architect` that
currently means Oswald for headings and IBM Plex Mono everywhere else
(`--personality-font-family` carries the personality's body font, which for
`architect` is also monospace).
`--lx-radius` adopts `--personality-border-radius`, and `--lx-focus` is simply
`var(--lx-accent)`.
`--lx-border-width` (`var(--personality-border-width, 2px)`) and `--lx-border-style`
(`var(--personality-border-style, solid)`) adopt the structural frame rules prescribed
by the personality.
`--lx-shadow-card` (`var(--personality-card-shadow, var(--shadow-lg))`),
`--lx-shadow-control` (`var(--personality-box-shadow, var(--shadow-md))`), and
`--lx-shadow-sm` (`var(--shadow-sm)`) adopt the personality's hard-offset dramatic
elevation profiles without hardcoding offsets. Recessed editors and execution wells
use `--lx-shadow-inset` (`var(--shadow-inset, ...)`) for a profile-aware tactile
depth with an SSR-safe fallback.
`--lx-page-pattern` (`var(--page-background-pattern)`) and `--lx-surface-texture`
(`var(--surface-texture)`) bring through the technical drafting blueprint grid and
cross-hatch textures generated by the personality engine.
`--lx-btn-transform`, `--lx-btn-weight`, and `--lx-btn-transition` adopt the uppercase,
bold, and stepped mechanical button interaction prescribed by the personality.
...
both typefaces, the corner radius, the structural border style and width, the hard-offset
elevation shadows, the blueprint page and surface patterns, the button uppercase/weight
prescriptions, and the focus ring. These are the traits that make the console feel
designed rather than merely dark, and they update automatically if the personality
system's definition of `architect` changes.
