  ...
  async search(topic: LeadTopic): Promise<ProviderSearchResult> {
    ...
    try {
      ...
      const results = rawResults
        .flat()
        .filter((result) => /indeed\.com\//i.test(result.url));
      ...
      return {
        ...
        warnings: candidates.length
          ? []
          : [
              'Indeed search returned no analyzable job leads for the configured topic.',
            ],
        queries,
      };
    } catch (error) {
      this.logger.warn(
        `Indeed discovery failed for topic ${topic.id}: ${
  ...
  private async mapResult(
    ...
    return {
      lead: createLeadEntity({
        ...
        notes: `Discovered via Indeed search. Source: ${
          result.url
        }. ${truncateText(
