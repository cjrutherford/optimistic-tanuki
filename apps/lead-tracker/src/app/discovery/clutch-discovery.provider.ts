  ...
  async search(topic: LeadTopic): Promise<ProviderSearchResult> {
    ...
    try {
      const rawResults = await Promise.all(
        queries.map((query) => this.searchAcquisitionService.searchWeb(query))
      );
      const results = rawResults
        .flat()
        .filter((result) => /clutch\.co\//i.test(result.url));
      ...
      return {
        ...
        warnings: candidates.length
          ? []
          : [
              'Clutch search returned no analyzable partner listings for the configured topic.',
            ],
        queries,
      };
    } catch (error) {
      this.logger.warn(
        `Clutch discovery failed for topic ${topic.id}: ${
  ...
  private async mapResult(
    ...
    return {
      lead: createLeadEntity({
        ...
        notes: `Discovered via Clutch partner search. Source: ${
          result.url
        }. ${truncateText(
