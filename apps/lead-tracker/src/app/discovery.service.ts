...
export class DiscoveryService {
  ...
  async getStatus(
    ...
    return (
      this.latestResults.get(topicId) || {
        topicId,
      ...
      try {
        ...
        if (topic) {
          this.latestResults.set(topicId, {
            ...
            lastRun: topic.lastRun?.toISOString(),
            providerResults:
              this.latestResults.get(topicId)?.providerResults || [],
        ...
        this.latestResults.set(topicId, {
          ...
          actionItems: [
            'Retry discovery after checking the topic configuration and provider settings.',
          ],
          diagnosticCounts: {
            errors: 1,
            warnings: 0,
  ...
  private async processTopicDiscovery(
    ...
    this.logger.log(
      `Starting discovery for topic ${topic.id} (${
        topic.name
      }) using sources: ${this.normalizeSources(topic.sources).join(', ')}`
    );
    ...
    const matchedLeadIds = new Set(
      matchedResults.map((entry) => entry.lead.id)
    );
    const autoLinks = existingLinks.filter((link) => link.linkType === 'auto');
    const manualLinks = existingLinks.filter(
      (link) => link.linkType !== 'auto'
    );
    const existingLinkLeadIds = new Set(
      existingLinks.map((link) => link.leadId)
    );
    ...
    const linksToRefresh = autoLinks.filter((link) =>
      matchedLeadIds.has(link.leadId)
    );
    for (const link of linksToRefresh) {
      const match = matchedResults.find(
        (entry) => entry.lead.id === link.leadId
      );
    ...
    const staleAutoLinks = autoLinks.filter(
      (link) => !matchedLeadIds.has(link.leadId)
    );
    ...
    const linkedLeadCount =
      manualLinks.length + linksToRefresh.length + newLinks.length;
    ...
    return {
      ...
      lead: {
        ...
        searchKeywords: Array.from(
          new Set([
            ...(existingLead.searchKeywords || []),
            ...(candidate.lead.searchKeywords || []),
          ])
        ),
      },
    };
  ...
  private buildLeadFingerprint(lead: Pick<Lead, 'name' | 'company'>): string {
    const normalizedCompany = this.normalizeFingerprintPart(
      lead.company || lead.name || ''
    );
    const normalizedName = this.normalizeFingerprintPart(
  ...
  private async enrichLeadContacts(lead: Lead): Promise<Lead> {
    ...
    return {
      ...lead,
      contacts,
      email: lead.email || selectPrimaryContactValue(contacts, 'email'),
      phone: lead.phone || selectPrimaryContactValue(contacts, 'phone'),
    };
  }
  ...
  private buildCompletionMessage(
    ...
    if (matchedCount > 0) {
      if (!warnings.length) {
        return `Discovery matched ${matchedCount} candidate lead${
          matchedCount === 1 ? '' : 's'
        }.`;
      }
  ...
  private buildDiscoveryDiagnostics(
    ...
    const issues = providerResults.flatMap(
      (providerResult) => providerResult.issues || []
    );
    ...
    if (matchedCount > 0 && issues.length) {
      return {
        ...
        summaryBody: `Discovery matched ${matchedCount} candidate lead${
          matchedCount === 1 ? '' : 's'
        }, but ${diagnosticCounts.providersWithIssues} provider${
  ...
  private determineProviderStatus(
    ...
    const issues = warnings.map((warning) =>
      this.classifyProviderIssue('provider', warning)
    );
  ...
  private classifyProviderIssue(
    ...
    if (normalizedWarning.includes('api key')) {
      return {
        ...
        detail: `${this.formatProviderLabel(
          providerName
        )} could not run because the API key is missing.`,
        action: `Add the ${this.formatProviderLabel(
          providerName
        )} API key before running this topic again.`,
      };
    }
    ...
    if (
      ...
      return {
        ...
        action: `Verify the ${this.formatProviderLabel(
          providerName
        )} endpoint or provider configuration before running again.`,
      };
    }
    ...
    if (normalizedWarning.includes('provider failed')) {
      return {
        ...
        detail: `${this.formatProviderLabel(
          providerName
        )} failed while running discovery.`,
        action: `Retry discovery after checking ${this.formatProviderLabel(
          providerName
        )} availability.`,
      };
    }
    ...
    if (
      ...
      return {
        ...
        detail:
          'Configured excluded terms filtered out the results returned by this provider.',
        action:
          'Review excluded terms if this topic is filtering out too many results.',
      };
    }
    ...
    if (
      ...
      return {
        ...
        detail: `${this.formatProviderLabel(
          providerName
        )} did not return results that matched this topic.`,
        action:
          'Broaden the topic keywords or source filters, then run discovery again.',
      };
    }
  ...
  private getProvidersForTopic(topic: LeadTopic): TopicDiscoveryProvider[] {
    ...
    const providers: TopicDiscoveryProvider[] = [
      this.internalDiscoveryProvider,
    ];
