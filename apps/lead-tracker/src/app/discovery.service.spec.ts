...
describe('DiscoveryService', () => {
    ...
    beforeEach(async () => {
        ...
        weWorkRemotelyProvider = { providerName: 'weworkremotely', search: jest.fn() };
    ...
    it('continues discovery when one provider fails', async () => {
        ...
        linkRepository.find.mockResolvedValue([]);
        internalProvider.search.mockResolvedValue(emptyProviderResult);
        remoteOkProvider.search.mockRejectedValue(new Error('provider unavailable'));
    ...
    it('surfaces missing-api-key diagnostics in the discovery message when no matches are found', async () => {
        ...
        expect(result?.actionItems).toContain('Add the Google Maps API key before running this topic again.');
