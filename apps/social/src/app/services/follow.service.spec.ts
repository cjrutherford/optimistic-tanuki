...
describe('FollowService', () => {
  ...
  describe('follow', () => {
    ...
    it('should throw if already following', async () => {
      followRepo.findOne.mockResolvedValueOnce({
        id: 'id2',
        followerId: 'a',
        followeeId: 'b',
        createdAt: new Date(),
      ...
      await expect(service.follow('a', 'b')).rejects.toThrow(
        'Already following'
      );
    });
  ...
  describe('unfollow', () => {
    it('should remove follow if exists', async () => {
      ...
      followRepo.findOne.mockResolvedValueOnce(null); // no inverse
      followRepo.remove.mockResolvedValue({
        id: 'id5',
        followerId: 'a',
        followeeId: 'b',
        createdAt: new Date(),
  ...
  it('getMutuals should call find with isMutual', async () => {
    ...
    expect(followRepo.find).toHaveBeenCalledWith({
      where: { followerId: 'a', isMutual: true },
    });
  });
