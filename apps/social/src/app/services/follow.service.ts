...
export default class FollowService {
  ...
  async follow(followerId: string, followeeId: string) {
    try {
      const currentFollow = await this.followRepo.findOne({
      ...
      const inverseFollow = await this.followRepo.findOne({
        where: { followerId: followeeId, followeeId: followerId },
      });
      if (inverseFollow) {
  ...
  async getMutuals(userId: string) {
    return await this.followRepo.find({
      where: { followerId: userId, isMutual: true },
    });
  }
