...
test.describe('Cross-App User Journey', () => {
  ...
  test('should register in owner-console and access all apps', async ({
    page,
  }) => {
    ...
    const bioInput = page.locator(
      'lib-text-input[formControlName="bio"] input'
    );
    ...
    await page.click(
      'otui-button[type="submit"] button, button:has-text("Register")'
    );
    ...
    const bioInputFOW = page.locator(
      'lib-text-input[formControlName="bio"] input'
    );
    ...
    await page
      ...
      .first()
      .setInputFiles('apps/full-stack-e2e/src/test-image.png');

    await page.click('otui-modal otui-button[variant="success"] button', {
      force: true,
    });
    ...
    const bioInputClient = page.locator(
      'lib-text-input[formControlName="bio"] input'
    );
