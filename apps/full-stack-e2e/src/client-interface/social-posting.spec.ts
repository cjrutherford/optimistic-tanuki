...
test.describe('Client Interface Social Posting', () => {
  ...
  test('First Party User can Post, Comment, Delete', async ({ page }) => {
    ...
    try {
        ...
        logger.error('Timeout waiting for redirect. Checking for error messages...');
        const errorText = await page.locator('.error, .alert, .validation-message').allTextContents();
    ...
    if (page.url().includes('settings')) {
        ...
        const nameInput = page.locator('lib-text-input[label="Profile Name"] input').or(page.locator('lib-text-input[label="Display Name"] input'));
        await nameInput.fill('Social Tester');
        await page.locator('lib-text-area[label="Bio"] textarea').fill('I am a tester');
        await page.getByRole('button', { name: 'Submit' }).or(page.getByRole('button', { name: 'Create Profile' })).click();
    ...
    // Tiptap editor
    await page.locator('.tiptap-editor-styles .ProseMirror').fill('This is a test post content.');
    await page.getByRole('button', { name: 'Post' }).click();
    logger.debug(`Post "${postTitle}" submitted. Step 4 Complete.`);

    // 5. Verify Post
    logger.debug('Step 5: Verify Post Visibility');
    const postLocator = page.locator('.post-container').filter({ hasText: postTitle }).first();
    await expect(postLocator).toBeVisible({ timeout: 15000 });
    // Profile name might take a moment to load asynchronously
    await expect(postLocator).toContainText('Social Tester', { timeout: 10000 }); 
    ...
    await postLocator.getByRole('button', { name: 'Leave a Comment' }).click({ force: true });
    logger.debug('Clicked Leave a Comment. Waiting for dialog...');
    
    const commentEditor = page.locator('.cdk-overlay-container quill-editor .ql-editor');
    await expect(commentEditor).toBeVisible({ timeout: 10000 });
    await commentEditor.fill('Nice post!');
    
    await page.locator('.cdk-overlay-container').getByRole('button', { name: 'Submit' }).click({ force: true });
    logger.debug('Comment submitted.');
    
    // Wait for the comment to appear in the DOM
    const commentLocator = postLocator.locator('.comment-content').filter({ hasText: 'Nice post!' });
    ...
    await postLocator.getByRole('button', { name: 'Delete' }).click({ force: true });
    
    // Verify post gone
    await expect(page.locator('.post-container').filter({ hasText: postTitle })).toHaveCount(0, { timeout: 15000 });
